...
# Start PostgreSQL vector database (pgvector)
docker compose up -d
...
- `POSTGRES_URL` for vector database (default: `postgresql://rag_user:rag_password@localhost:5432/rag_db`)
...
- PostgreSQL with pgvector runs in Docker (configured in `docker-compose.yaml`)
...
2. **retrieve**: Performs hybrid search using dense vectors + full-text search via `VectorDB.hybrid_search()`
...
- **Dense vectors**: `intfloat/multilingual-e5-large-instruct` (1024 dimensions) via Together AI, stored using pgvector with HNSW index
...
Falls back to dense-only search if no full-text matches are found.
...
3. **Embed**: `generate_embeddings()` creates dense embeddings with retry logic (tsvector generated by PostgreSQL)
4. **Store**: `store_documents()` upserts to PostgreSQL with UUID-based deduplication (`_generate_point_id()`)
...
- `GET /health`: Health check with PostgreSQL table name
